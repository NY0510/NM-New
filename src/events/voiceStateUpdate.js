			...
			await wait(() => {
				if (player.paused && stateChange.members.size === 0) {
					...
					client.channels.cache
						...
						.send({ embeds: [new EmbedBuilder().setTitle("👋 음성채널이 10분동안 비어있어서, 음악을 종료하고 퇴장했어요").setColor(client.config.color.normal)] });
				}
		...
		switch (stateChange.type) {
			case "JOIN":
				if (stateChange.members.size === 1 && player.paused) {
					client.channels.cache.get(String(player.textChannel)).send({ embeds: [new EmbedBuilder().setTitle("▶️ 일시정지를 해제했어요").setColor(client.config.color.normal)] });
			...
			case "LEAVE":
				if (stateChange.members.size === 0 && !player.paused && player.playing) {
					client.channels.cache
						.get(String(player.textChannel))
						.send({ embeds: [new EmbedBuilder().setTitle("⏸️ 음성채널이 비어있어서, 음악을 일시정지 했어요").setColor(client.config.color.normal)] });
